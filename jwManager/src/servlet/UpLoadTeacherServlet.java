	  		...
	  		while(iter.hasNext())
	  		{
	  			...
	  			else
	  			{
	  				...
	  				{ 
							...
							out.println("</script>");
							if(td.insertDB(id, filename, filepath)>0)
							{
								out.println("<script language=javascript>");
								out.println("alert('插入数据库成功');");
								out.println("</script>");
								response.sendRedirect("SelectAllTeachersServlet");
							}
							else{
								out.println("<script language=javascript>");
								out.println("alert('插入数据库失败');");
								out.println("window.location.href='insertTech.jsp'");
								out.println("</script>");
							}
							out.flush();
							out.close();
